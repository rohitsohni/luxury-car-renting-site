...
const Navbar = () => {
  ...
  return (
    ...
    className={`flex items-center justify-between px-6 md:px-16 lg:px-24 xl:px-32 py-4 border-b border-borderColor relative z-40 transition-all ${location.pathname === "/" ? "bg-white/75 text-asphalt backdrop-blur-xl" : "bg-white text-gray-700"}`}>
        ...
        <div className={`max-sm:fixed max-sm:h-screen max-sm:w-full max-sm:top-16 max-sm:border-t border-borderColor right-0 flex flex-col sm:flex-row items-start sm:items-center gap-4 sm:gap-8 max-sm:p-4 transition-all duration-300 z-50 ${location.pathname === "/" ? "bg-white/95 sm:bg-transparent" : "bg-white"} ${open ? "max-sm:translate-x-0" : "max-sm:translate-x-full"}`}>
            ...
            <div className='flex max-sm:flex-col items-start sm:items-center gap-6'>
                ...
                <button onClick={()=> isOwner ? navigate('/owner') : changeRole()} className="cursor-pointer font-medium transition hover:text-primary">{isOwner ? 'Dashboard' : 'List cars'}</button>
